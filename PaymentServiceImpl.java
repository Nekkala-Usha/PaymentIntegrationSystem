	...
	public CreatePaymentResDTO processPayment(CreatePaymentReqDTO paymentReq) {
		...
		if (response.getStatusCode().is2xxSuccessful()) {
			...
			if(session.getUrl() != null 
					&& !session.getUrl().trim().isEmpty()) { // Regex
				//Valid URL came.
			...
			throw new StripeProviderException(
					...
					HttpStatus.BAD_GATEWAY);//HTTP Status

		}
		
		if(HttpStatus.GATEWAY_TIMEOUT == response.getStatusCode()) {
			//TODO handle Gateway time 
			throw new StripeProviderException(
		...
		StripeErrorWrapper errorResponse = gson.fromJson(
				response.getBody(), StripeErrorWrapper.class);
		
		System.out.println("Got error response from Stripe| errorResponse:"
				+ "" + errorResponse);
