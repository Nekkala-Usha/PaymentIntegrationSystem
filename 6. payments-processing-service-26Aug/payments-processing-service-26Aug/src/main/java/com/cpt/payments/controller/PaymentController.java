...
public class PaymentController { 
	...
	@PostMapping(value = {EndPoints.EMPTY_STRING})
    public ResponseEntity<CreateTransactionRes> createPayment(@RequestBody Transaction transaction) {
		...
		System.out.println("Transaction object after conversion"
				+ "|txnDTO:" + txnDTO);
    ...
    public ResponseEntity<InitiatePaymentRes> initiatePayment(
		...
		InitiatePaymentReqDTO reqDTO =modelMapper.map(
				paymentReq, InitiatePaymentReqDTO.class);
		reqDTO.setId(id);
		...
		System.out.println("Got response service"
